...
<<<<<<< HEAD
  ...
  if (!helper.detectIsEmpty(user).flag) {
    ...
    try {
      ...
      // 并且返回插入用户的信息
      let sqlRes_openid = await sql.query(
        "select * from user WHERE nickname = ?",[user.nickName]);
      ...
      if (!isNew) {
        ...
        let sqlRes = await sql.query(
          "insert into user values (null,?,?,?,?,?)",[user.nickName,user.sex,user.phone,user.email,user.password]);
